...
# ══════════════════════════════════════════════════════════════════════════════
# GRAPHICS3D → matplotlib PNG  (Bugs #13, #14)
# ══════════════════════════════════════════════════════════════════════════════
...
#
# Γιατί έδειχνε "[3D — pip install matplotlib numpy]" παλιότερα:
# Τα VB-wrapped cells (FrontEndRef["uuid"]) δεν αναλύονταν σωστά,
# με αποτέλεσμα το g3d_to_b64_matplotlib() να μην καλείται ποτέ.
# Μετά τη διόρθωση του VB handler και του gmap UUID resolution, τα
# Graphics3D αντικείμενα φτάνουν κανονικά στη συνάρτηση.
#
...
def _wl_num(v):
    """Μετατρέπει WL numeric expression tree σε Python float. (Bug #06)
...
def tp(x,y,xmn,xmx,ymn,ymx,W,H,pad):
    ...
    pw/ph υπολογίζονται τοπικά — πρώην χρησιμοποιούσε globals που δεν ορίζονταν
    όταν η tp() κλήθηκε πριν τεθούν τα pw/ph στο graphics_to_svg (bug fix)."""
...
def bar_graphics_to_svg(bar_g, vmin, vmax, H=300, grad_id='cbgrad'):
    ...
    Bug #14: Χρησιμοποιείται για τη BarLegend που εμφανίζεται δίπλα στο StreamPlot.
...
def render_md(text):
    ...
    ctr = [0]  # mutable counter for unique placeholder keys (bug fix: was undefined)
    def sub(m):
    ...
    for cell in cells_list[1:]:
        ...
        #   - plain string: αριθμός, σύμβολο, αποτέλεσμα χωρίς formatting
            ...
            if 'temporal$' in str(data) and not re.search(r'[0-9a-f]{8}-[0-9a-f]{4}', str(data)):
                ...
                #   - LineLegend (Plot) → parse_linelegend + linelegend_to_svg
                #   - BarLegend (StreamPlot, VectorPlot) → bar_graphics_to_svg
                # Η legend εμφανίζεται δίπλα στο γράφημα με display:flex.
        ...
        # ── Input codemirror hidden=False: Wolfram code ───────────────────────
            ...
            if is_fade:
                # Collapsible (Fade=True): μισο-κρυμμένο, κλικ για ανάπτυξη
                blocks.append(
